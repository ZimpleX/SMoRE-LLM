...
def get_peft_model_state_dict(
    ...
    # TUNER SPECIFIC CODE
    if config.peft_type in (PeftType.HIER_MOE, PeftType.HYDRALORA):
        if config.peft_type == PeftType.HIER_MOE:
            prefix = "moe_"
        elif config.peft_type == PeftType.HYDRALORA:
            prefix = "hydralora_"
        to_return = {k: state_dict[k] for k in state_dict if prefix in k}
        to_return = {k: v for k, v in to_return.items() if prefix in k and adapter_name in k}
    ...
    for key, val in state_dict.items():
        if parameter_prefix in key:
            ...
            if "." in suffix:
                ...
                key = f"{adapter_name}.{suffix_to_replace}".join(key.rsplit(suffix_to_replace, 1))
...
def set_peft_model_state_dict(
    ...
    if config.peft_type in PEFT_TYPE_TO_PREFIX_MAPPING:
        ...
        elif config.peft_type == PeftType.HIER_MOE:
            pass
        elif config.peft_type == PeftType.HYDRALORA:
            pass
