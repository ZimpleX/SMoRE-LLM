...
def check_file_exists_on_hf_hub(repo_id: str, filename: str, **kwargs) -> Optional[bool]:
    ...
    if EntryNotFoundError is None:
        pass_exceptions = HFValidationError
    else:
        pass_exceptions = (HFValidationError, EntryNotFoundError)
    try:
        exists = file_exists(repo_id, filename, **kwargs)
    except pass_exceptions:  # NOTE should also include this EntryNotFoundError
