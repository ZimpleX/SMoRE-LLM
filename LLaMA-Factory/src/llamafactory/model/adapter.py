...
import torch
from peft import PeftModel, TaskType, get_peft_model, HieRMoEConfig
...
def _setup_moe_tuning(
    ...
    if is_trainable:
        logger.info_rank0(f"Fine-tuning method: {moe_type}")

    assert model_args.adapter_name_or_path is None
    assert is_trainable
    assert moe_type == "smore"
    moe_config = HieRMoEConfig(
        task_type=TaskType.CAUSAL_LM,
        inference_mode=False,
        num_experts=finetuning_args.moe_num_experts,
        num_active=finetuning_args.moe_num_active,
        expert_dims=finetuning_args.moe_expert_dims,
        lora_alpha=finetuning_args.moe_lora_alpha,
        lora_scaling=finetuning_args.moe_lora_scaling,
        gate_dims=finetuning_args.moe_gate_dims,
        gate_type=finetuning_args.moe_gate_type,
        gate_arch=finetuning_args.moe_gate_arch,
        balance_loss_weight=finetuning_args.moe_balance_loss_weight,
        dim_downproj_x=finetuning_args.moe_dim_downproj_x,
        init_method=finetuning_args.moe_init_method,
        gate_act_fn=finetuning_args.moe_gate_act_fn,
        expert_act_fn=finetuning_args.moe_expert_act_fn,
        moe_arch=finetuning_args.moe_arch,
        target_modules=finetuning_args.moe_target_modules,
        exclude_modules=finetuning_args.moe_exclude_modules,
        is_shared_expert_pool=finetuning_args.moe_is_shared_expert_pool,
        activation_position=finetuning_args.moe_activation_position,
    )
...
def init_adapter(
    ...
    #     )
    elif finetuning_args.finetuning_type in ["peft_moe", "peft_hydralora"]:
        if finetuning_args.finetuning_type == "peft_moe":
            moe_type = "smore"
        else:
            moe_type = "hydralora"
        model = _setup_moe_tuning(
            config, 
            model, 
            model_args, 
            finetuning_args, 
