...
if TYPE_CHECKING:
    ...
    from ..hparams import DataArguments, ModelArguments, CustomizedSeq2SeqTrainingArguments
...
def _load_single_dataset(
    ...
    if dataset_attr.local_subset is not None:
        assert not data_args.streaming, 'streaming not supported yet for local subsetting'
        LOCAL_SUBSET_DIR = 'local_subsets'
        import json
        with open(os.path.join(dataset_attr.dataset_dir, LOCAL_SUBSET_DIR, f'{dataset_attr.local_subset}.json')) as fi:
            local_subset_indices = json.load(fi)
            dataset = dataset.select(local_subset_indices)
