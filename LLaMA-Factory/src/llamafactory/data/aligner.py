...
if TYPE_CHECKING:
    ...
    from ..hparams import CustomizedSeq2SeqTrainingArguments
...
def convert_choice(
    ...
    r"""
    Converts choice format dataset to the standard format.
    """
    prompt = []

    # question
    query = ["Please choose the correct answer to the question: " + example[dataset_attr.prompt]]
    
    choices = example[dataset_attr.query]
    text_list = choices["text"]
    label_list = choices["label"]
    if len(text_list) != len(label_list):
        raise Exception("text list and label list are not the same!")
    else:
        for i in range(len(text_list)):
            query.append(f" ({label_list[i]}) {text_list[i]} ")

    prompt.append({"role": Role.USER.value, "content": "".join(query)})  # "prompt\nquery"
    response = [{"role": Role.ASSISTANT.value, "content": example[dataset_attr.response]}]
...
def convert_two_options(
    ...
    """
    prompt = []
    query = "Please choose the correct answer to fill in the blank to complete the given sentence.\n"
    query += f"Sentence: {example[dataset_attr.prompt]}" # sentence
    query += f"\n(A) {example[dataset_attr.option1]}\n(B) {example[dataset_attr.option2]}"
    answer = example[dataset_attr.response] # '1' or '2'
    if answer == '1':
        label = 'A'
    elif answer == '2':
        label = 'B'
    else:
        raise ValueError(f"label must be 1 or 2, label is {answer}")

    prompt.append({"role": Role.USER.value, "content": query})
    response = [{"role": Role.ASSISTANT.value, "content": label}]
...
def align_dataset(
    ...
    elif dataset_attr.formatting == "choice":
        convert_func = partial(convert_choice, dataset_attr=dataset_attr, data_args=data_args)
    elif dataset_attr.formatting == "two_options":
        convert_func = partial(convert_two_options, dataset_attr=dataset_attr, data_args=data_args)
    else:
