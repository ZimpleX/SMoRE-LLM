...
from .model_args import CustomizedSeq2SeqTrainingArguments
from .evaluation_args import EvaluationArguments
from .finetuning_args import FinetuningArguments
from .generating_args import GeneratingArguments
from .model_args import ModelArguments, DummyArguments
...
_TRAIN_ARGS = [ModelArguments, DataArguments, CustomizedSeq2SeqTrainingArguments, FinetuningArguments, GeneratingArguments, DummyArguments]
_TRAIN_CLS = Tuple[ModelArguments, DataArguments, CustomizedSeq2SeqTrainingArguments, FinetuningArguments, GeneratingArguments]
...
def _parse_args(parser: "HfArgumentParser", args: Optional[Dict[str, Any]] = None) -> Tuple[Any]:
    ...
    if len(sys.argv) == 2 and (sys.argv[1].endswith(".yaml") or sys.argv[1].endswith(".yml")):
        ret = parser.parse_yaml_file(os.path.abspath(sys.argv[1]))
        data_args = ret[1]
        trainer_args = ret[2]
        finetuning_args = ret[3]
        if hasattr(trainer_args, "amend_logs_by_data_args"):
            trainer_args.amend_logs_by_data_args(data_args)
        if hasattr(trainer_args, "amend_logs_by_finetuning_args"):
            trainer_args.amend_logs_by_finetuning_args(finetuning_args)
        return ret
...
def _verify_model_args(
    ...
    if model_args.adapter_name_or_path is not None and finetuning_args.finetuning_type not in ("lora", "peft_moe", "peft_hydralora", "llama_moe"):
...
def _check_extra_dependencies(
    model_args: "ModelArguments",
    finetuning_args: "FinetuningArguments",
    training_args: Optional["CustomizedSeq2SeqTrainingArguments"] = None,
) -> None:
...
def get_train_args(args: Optional[Dict[str, Any]] = None) -> _TRAIN_CLS:
    ...
    if (
        ...
        and finetuning_args.finetuning_type in ("lora", "peft_moe", "peft_hydralora", "llama_moe")
